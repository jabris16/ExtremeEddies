...
def eddytype_subset(eddies): 
    
    '''
    
    Function for subsetting the complete 'eddies' dataset [ output from eddy_metrics() function ] into subsets containing only cyclonic/anticyclonic/anticyclonic-modewater (ACME) eddies.
    
    INPUTS:
    eddies = complete dataset of tracked eddy information including additional metrics [ output form eddy_metrics() function ] 
    
    OUTPUTS:
    eddies_anti = subset containing only anticyclonic eddy information
    eddies_cyc = subset containing only cyclonic eddy information
    eddies_acme = subset containing only ACME information
        METRICS:
        'time' = time index of eddy feature in the analysis period
        'type' = anticyclonic or cyclonic or ACME eddy
        'age' = total age of eddy [days]
    ...
    # further split into anticylonic, cyclonic and ACME datasets
    for ed in range(len(eddies)):
        eddy_int = {}
        eddy_int['time'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['amp'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['scale'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['rot_velocity'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['lat'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['lon'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['rot_velocity'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['translation_speed'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['nonlin'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['sss_anom'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['sst_anom'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['mld'] = np.zeros(len(eddies[ed]['lon']))
        eddy_int['area'] = np.zeros(len(eddies[ed]['lon']))
        if eddies[ed]['type'] == 'cyclonic':
            for x in range(len(eddies[ed]['lon'])):
                eddy_int['time'][x] = eddies[ed]['time'][x]
                eddy_int['sss_anom'][x] = eddies[ed]['sss_anom'][x]
                eddy_int['sst_anom'][x] = eddies[ed]['sst_anom'][x]
                eddy_int['mld'][x] = eddies[ed]['mld'][x]
                eddy_int['amp'][x] = eddies[ed]['amp'][x]
                ...
                eddy_int['area'][x] = eddies[ed]['area'][x]
            eddies_cyc.append(eddy_int)
        if eddies[ed]['type'] == 'acme':
            for x in range(len(eddies[ed]['lon'])):
                eddy_int['time'][x] = eddies[ed]['time'][x]
                eddy_int['sss_anom'][x] = eddies[ed]['sss_anom'][x]
                eddy_int['sst_anom'][x] = eddies[ed]['sst_anom'][x]
                ...
                eddy_int['nonlin'][x] =  eddies[ed]['nonlin'][x]
                eddy_int['area'][x] = eddies[ed]['area'][x]
            eddies_acme.append(eddy_int)
        if eddies[ed]['type'] == 'anticyclonic':
            for x in range(len(eddies[ed]['lon'])):
                eddy_int['time'][x] = eddies[ed]['time'][x]
                eddy_int['sss_anom'][x] = eddies[ed]['sss_anom'][x]
                eddy_int['sst_anom'][x] = eddies[ed]['sst_anom'][x]
                ...
                eddy_int['nonlin'][x] =  eddies[ed]['nonlin'][x]
                eddy_int['area'][x] = eddies[ed]['area'][x]
            eddies_anti.append(eddy_int)
            
    return eddies_cyc, eddies_acme, eddies_anti
